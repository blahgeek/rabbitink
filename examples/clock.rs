...
fn main() -> anyhow::Result<()> {
    ...
    let y_repeat = if args.repeat > 0 {args.repeat} else {dev.get_screen_size().height / args.height};

    let mut img : cv::core::Mat1b = cv::core::Mat::new_rows_cols_with_default(
        args.height, screen_size.width, cv::core::CV_8UC1, cv::core::Scalar::all(0xf0 as f64))?.try_into_typed()?;
    for n in 0.. {
        ...
        info!("clock: {}, start", n);
        for y in 0..y_repeat {
            dev.load_image_area((0, args.height * y).into(), &img)?;
        }
        ...
        for y in 0..y_repeat {
            // dev.set_image_buf_idx(y as u32 % 5);
            dev.display_area(opencv::core::Rect2i::new(0, args.height * y, screen_size.width, args.height),
                             args.mode,
                             false)?;
        }
